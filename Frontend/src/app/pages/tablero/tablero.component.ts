...
export class TableroComponent implements OnInit, OnDestroy {
      ...
      next: (result: PartidaDto) => {
        if (result.estado === EstadoPartida.TERMINADA) {
          this.tableroServicio.consultarMotivoGanador(result.ganador.idJugador).subscribe({
            ...
            error: (err) => console.error('Error al consultar el motivo del ganador:', err)
          });
          return;
        }
        ...
        const jugador: JugadorDto | undefined = this.partida.jugadores?.find(j => j.idJugador === turno.idJugador);
        ...
        this.jugadorConsquisto = this.partida.jugadores?.find(
          j => j.consquisto && j.idUsuario === usuarioActual?.idUsuario && turno.fase === FaseTurno.MOVER_TROPAS
        );

        const jugadorUsuario = this.partida.jugadores?.find(j => j.idUsuario === usuarioActual.idUsuario);
        ...
        for (const tarjeta of this.cartasJugador) {
          tarjeta.jugadorTienePais = this.partida.estadoPaises.some(
            p => p.idJugador === jugadorUsuario.idJugador && p.pais.idPais === tarjeta.tarjeta.pais.idPais && !tarjeta.usada
          );
        }
  ...
  avanzarFaseTurno() {
    this.tableroServicio.cambiarTurno(this.partida.idPartida).subscribe({
      error: (err) => console.error('Error al avanzar la fase del turno:', err)
    });
  }
  ...
  paisClickeado(estadoPais: EstadoPaisDto) {
    this.dialog.open(AccionesPaisComponent, {
      data: { paisSeleccionado: estadoPais },
      panelClass: 'teg-dialog',
    });
  }
  ...
  obtenerColor(color: string): string {
    switch (color?.toLowerCase()) {
      ...
      case 'verde':    return '#009688';
      case 'azul':     return '#3f51b5';
      case 'amarillo': return '#ffc107';
      case 'violeta':  return '#9c27b0';
