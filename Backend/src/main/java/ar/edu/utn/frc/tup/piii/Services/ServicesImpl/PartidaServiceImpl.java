...
public class PartidaServiceImpl implements PartidaService {
    ...
    @Autowired private EstadoTarjetaService estadoTarjetaService;
    @Autowired private ModelMapper modelMapper;
    @Autowired private UsuarioRepository usuarioRepository;
    @Autowired private EstadisticaService estadisticaService;
    @Autowired private EstadoPaisService estadoPaisService;
    @Autowired private ObjetivoService objetivoService;
    ...
    public Partida crearPartidaYAsignarJugadores(Long idSala, Usuario usuarioActual) {
        ...
        if (!sala.getCreador().getIdUsuario().equals(usuarioActual.getIdUsuario())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Solo el creador de la sala puede iniciar la partida.");
        }
    ...
    public PartidaDto cargarPartidaBySala(String url, Long idUsuario) {
        SalaEntity sala = salaRepository.findByUrl(url)
                .orElseThrow(() -> new IllegalArgumentException("No se encontró la sala con URL: " + url));
        ...
        PartidaEntity partida = partidaRepository.findByConfiguracion_IdSala(sala.getIdSala())
                .orElseThrow(() -> new IllegalArgumentException("No se encontró la partida activa para esta sala."));
    ...
    public PartidaDto cargarPartida(Long idPartida) {
        PartidaEntity partidaEntity = partidaRepository.findById(idPartida)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Partida con ID " + idPartida + " no encontrada"));
        ...
        partida.setTurnoActual(partidaEntity.getTurnoActual());
        partida.setGanador(
                partidaEntity.getGanador() != null ? modelMapper.map(partidaEntity.getGanador(), JugadorDto.class) : null
        );
...
//        );
        ...
        partida
                ...
                partidaEntity.getEstadoPaises().stream()
                        .map(e -> new EstadoPaisDto().builder()
                                ...
                                .pais(
                                        new Pais().builder()
                                                .idPais(e.getPais().getIdPais())
                                                        .nombre(e.getPais().getNombre())
                                                                .continente(
                                                                    new Continente().builder()
                                                                            .idContinente(e.getPais().getContinente().getIdContinente())
                                                                            .nombre(e.getPais().getContinente().getNombre())
                                                                            .build()
                                                                )
                                                .build()
                                )
        ...
        partida.setEstadoTarjetas(
                partidaEntity.getMazo().stream()
                        .map(et -> new EstadoTarjetaDto().builder()
                                ...
                                .idJugador(et.getJugador() != null ? et.getJugador().getIdJugador() : null)
                                .tarjeta(new TarjetaDto().builder()
                                        .idTarjeta(et.getTarjeta().getIdTarjeta())
                                        .simbolo(String.valueOf(et.getTarjeta().getSimbolo()))
                                        .pais(modelMapper.map(et.getTarjeta().getPais(), Pais.class))
                                        .build())
                                .usada(et.isUsada())
    ...
    public Partida obtenerEstadoPartida(Long idPartida) {
        PartidaEntity partida = partidaRepository.findById(idPartida)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Estado de partida con ID " + idPartida + " no encontrada"));
        return modelMapper.map(partida, Partida.class);
    }
    ...
    public void verificarCondicionVictoria(Partida partida) {
        ...
        List<JugadorEntity> jugadoresActivos = jugadorRepository.findByPartidaAndEstadoJugador(partidaEntity, EstadoJugador.ACTIVO);
